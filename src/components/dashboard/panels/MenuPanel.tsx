...
export default function MenuPanel({ truck }: { truck: Truck }) {
  ...
  const lastSaved = useRef<string>(JSON.stringify(serialize(items)));

  // Debounced autosave whenever the menu changes.
  useEffect(() => {
    if (firstRender.current) {
      firstRender.current = false;
      return;
    }
    const payload = JSON.stringify(serialize(items));
  ...
  const priceCell =
    "w-28 rounded-lg border border-line bg-card py-1.5 pl-10 pr-2 text-right font-mono text-sm text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/20";
